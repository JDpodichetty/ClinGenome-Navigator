...
class LLMProcessor:
        ...
        try:
            ...
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": """You are a clinical genomics research assistant specializing in kidney disease and genetic variants. 
    ...
    def extract_clinical_insights(self, clinical_notes: List[str], patient_data: Dict = None) -> Dict:
        ...
        try:
            ...
            notes_text = "\n".join(clinical_notes[:50])  # Limit to avoid token limits
            ...
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": """You are a clinical data analyst specializing in nephrology and genetic medicine. 
                    ...
                    {
                        "role": "user",
                        "content": f"Clinical Notes to Analyze:\n{notes_text}{patient_context}"
                    }
                ],
                response_format={"type": "json_object"},
                max_tokens=1500
    ...
    def generate_smart_query_suggestions(self, df: pd.DataFrame) -> List[str]:
        ...
        try:
            ...
            summary_data = {
                ...
                "diagnoses": df['Diagnosis'].value_counts().head(5).to_dict() if 'Diagnosis' in df.columns else {},
                "ethnicities": df['Ethnicity'].value_counts().to_dict() if 'Ethnicity' in df.columns else {},
                "age_range": {
                    "min": int(df['Age'].min()),
                    "max": int(df['Age'].max()),
            ...
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": """You are a clinical research strategist. Generate 8-10 intelligent, specific query suggestions 
    ...
    def analyze_patient_cohort(self, filtered_df: pd.DataFrame, query: str) -> Dict:
        ...
        try:
            ...
            cohort_data = {
                ...
                "demographics": {
                    "age_stats": {
                        "mean": round(filtered_df['Age'].mean(), 1),
                        "median": round(filtered_df['Age'].median(), 1),
                        "range": f"{int(filtered_df['Age'].min())}-{int(filtered_df['Age'].max())}"
                    } if 'Age' in filtered_df.columns else {},
                    "sex_distribution": filtered_df['Sex'].value_counts().to_dict() if 'Sex' in filtered_df.columns else {},
                    "ethnicity_distribution": filtered_df['Ethnicity'].value_counts().to_dict() if 'Ethnicity' in filtered_df.columns else {}
                },
                "clinical_metrics": {
                    "egfr_stats": {
                    ...
                    "diagnoses": filtered_df['Diagnosis'].value_counts().to_dict() if 'Diagnosis' in filtered_df.columns else {},
                    "trial_eligible": filtered_df['Eligible_For_Trial'].value_counts().to_dict() if 'Eligible_For_Trial' in filtered_df.columns else {}
                }
            }
            ...
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": """You are a clinical research analyst. Analyze patient cohorts to provide actionable insights 
    ...
    def generate_research_summary(self, df: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            if 'Clinical_Notes' in df.columns:
                clinical_notes_sample = df['Clinical_Notes'].dropna().head(20).tolist()
            ...
            dataset_overview = {
                ...
                "demographics": {
                    "age_distribution": {
                        ...
                        "range": f"{int(df['Age'].min())}-{int(df['Age'].max())}"
                    } if 'Age' in df.columns else {},
                    "sex_breakdown": df['Sex'].value_counts().to_dict() if 'Sex' in df.columns else {},
                    "ethnicity_breakdown": df['Ethnicity'].value_counts().to_dict() if 'Ethnicity' in df.columns else {}
                },
                "clinical_characteristics": {
                    "diagnoses": df['Diagnosis'].value_counts().head(10).to_dict() if 'Diagnosis' in df.columns else {},
                    "egfr_stats": {
                        "mean": round(df['eGFR'].mean(), 1),
                        "severe_dysfunction": len(df[df['eGFR'] < 30]),
                        "normal_function": len(df[df['eGFR'] >= 90])
                    } if 'eGFR' in df.columns else {},
                    "trial_eligibility": df['Eligible_For_Trial'].value_counts().to_dict() if 'Eligible_For_Trial' in df.columns else {}
                },
            ...
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": """You are a senior clinical research director providing strategic insights on clinical genomics datasets 
                        for pharmaceutical research and drug development.
                    ...
                    {
                        "role": "user",
                        "content": f"Clinical Genomics Dataset Overview:\n{json.dumps(dataset_overview, indent=2)}"
                    }
                ],
                response_format={"type": "json_object"},
                max_tokens=2000
